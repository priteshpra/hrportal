<script type="text/javascript">
   $(document).ready(function () {
    ...
    window.total_page = 1;
    window.EthnicityName = '';
    window.Status = '-1';
    function common_ajax (current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/ethnicity/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
            ...
            success: function (data)
            {
                ...
                $('#ethnicity_table_body').html(obj.a);
    ...
    $(document).ready(function () {
        ...
        $("#model_title").html("<?php echo label('msg_lbl_ethnicity');?>");
        common_ajax(current_page_size,total_page);
    })

    $('#button_submit').on('click', function () {
       EthnicityName =  $('#EthnicityName').val();  
 ...
 var field_name = ["EthnicityName"]; 
   $("#ethnicity_table_body").on('click', '.status_change', function()
   {
        ...
        $.ajax({
            type:"post",
            url: base_url + "admin/masters/ethnicity/changeStatus",
    ...
    $("#ethnicity_table_body").on('click', '.info', function(){ //$(".info").on('click', function () {
        var id = $(this).attr('data-id');
        var table_name = "ssc_ethnicity";
        var field_name = "EthnicityID";
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/ethnicity/getRecordInfo",
