<script type="text/javascript">
   $(document).ready(function () {
    ...
    function common_ajax (current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/skill/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
                    SkillName: SkillName,  
 ...
 var field_name = ["SkillName"]; 
   $("#skill_table_body").on('click', '.status_change', function()
   {
    ...
    $("#skill_table_body").on('click', '.info', function(){ //$(".info").on('click', function () {
        var id = $(this).attr('data-id');
        var table_name = "ssc_skill";
        var field_name = "skillID";
        $.ajax({
            type: "post",
