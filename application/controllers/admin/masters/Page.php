...
class Page extends Admin_Controller {
    ...
    public function index($per_page_record = 10, $page_number = 1) {
        try {
            ...
            $this->load->view('admin/masters/page/list', $res);
            $data = array();
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1) {
        ...
        $result['page'] = $this->page_model->listPage($per_page_record, $page_number);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $res = $this->page_model->insertPage($this->input->post());
                    if (@$res->ID) {
        ...
        try {
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->page_model->updatePage($caregory_array);
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/page');
                    } else {
                        $error_array = array(
            ...
            $this->load->view('admin/includes/header');
            $res = $data = array();
            $res['page_name'] = 'edit/' . $page_id;
            $res['page'] = $this->page_model->getPageDetailsByID($page_id);
            if(empty($res['page'])){
                $this->session->set_flashdata('posterror', label('record_not_found'));
                redirect($this->config->item('base_url') . 'admin/masters/page/');
            }
            $res['loading_button'] = getLoadingButton();
            $this->load->view('admin/masters/page/add_edit', $res);
            $data['page_level_js'] = $this->load->view('admin/masters/page/add_edit_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $data);
            unset($res, $data);
        } catch (Exception $e) {
    ...
    function changeStatus() {
        try {
            if ($this->input->post()) {
                $this->page_model->changeStatus($this->input->post());
            }
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function export_to_excel() {
        ...
        $res['page'] = $this->page_model->listpage(-1, 1);
