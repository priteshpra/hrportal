...
class Category extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_A_GetRoleMappingByID(" . $this->UserRoleID . ",13)");
        $tmp->next_result();
    ...
    public function index() {
		...
		$this->load->view('admin/masters/category/list', $res);
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1) {
        ...
        $result['category'] = $this->category_model->listData($per_page_record, $page_number);
        if (empty($result['category']))
            $result['total_records'] = 0;
        else
            $result['total_records'] = $result['category'][0]->rowcount;

        $result['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
        if ($result['total_records'] != 0) {
            $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
            $ajax_listing = $this->load->view('admin/masters/category/ajax_listing', $result, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
				...
				if ($this->form_validation->run() == TRUE) {
                    $url = site_url("admin/masters/category/add");
                    $config = array("max_width" => CATEGORY_MAX_WIDTH,
                        "max_height" => CATEGORY_MAX_HEIGHT,
                        'max_size' => CATEGORY_MAX_SIZE,
                        'path' => CATEGORY_UPLOAD_PATH,
                        'allowed_types' => CATEGORY_ALLOWED_TYPES,
                        'tpath' => CATEGORY_THUMB_UPLOAD_PATH,
                        'twidth' => CATEGORY_THUMB_MAX_WIDTH,
                        'theight' => CATEGORY_THUMB_MAX_HEIGHT
                    );
                    ...
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/category');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/masters/category/add');
                    }
                } else {
            ...
            $array['page_level_js'] = $this->load->view('admin/masters/category/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $url = site_url("admin/masters/category/edit/".$ID);
                    $config = array("max_width" => CATEGORY_MAX_WIDTH,
                        "max_height" => CATEGORY_MAX_HEIGHT,
                        'max_size' => CATEGORY_MAX_SIZE,
                        'path' => CATEGORY_UPLOAD_PATH,
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/category/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/category/edit/' . $ID);
            ...
            $data['loading_button'] = getLoadingButton();
			$data['cities'] = getCityCombobox(@$data['category']->CountryID);
            $this->load->view('admin/masters/category/add_edit', $data);
    ...
    public function export_to_excel() {
        ...
        $array = array();
        $fields = array("SrNo", "CategoryName","Description"); //Header Define
