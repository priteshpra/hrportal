...
class Unregisteruser extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/unregister_model');
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/unregisteruser/list', $res);
        $data['page_level_js'] = $this->load->view('admin/masters/unregisteruser/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/unregisteruser/add';
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['candidate'] = $this->unregister_model->listData($per_page_record, $page_number);
        ...
        if ($result['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing', $result, TRUE);
    ...
    public function details($ID = 0)
    {
        ...
        $data['details'] = $this->unregister_model->getCandidateByID($ID);
        if (!@$data['details']->ID) {
            show_404();
        }
        ...
        $data['page_level_js'] = $this->load->view('admin/masters/unregisteruser/details_js', $data, TRUE);
        $this->load->view('admin/includes/header');
        $this->load->view('admin/masters/unregisteruser/details', $data);
        $this->load->view('admin/includes/footer', $data);
        unset($data);
    }
    ...
    {
        $details = $this->unregister_model->getCandidateByID($id);

        $userData = array(
            'Password' => md5($details->FirstName),
            'EmailID'    => $details->EmailID,
            'MobileNo' => $details->ContactNumber,
            'UserType' => 'Candidate',
            'RegistrationType' => 'RegistrationType'
        );
        $insert_id = $this->unregister_model->insert_user($userData);
        ...
        $data = array(
            ...
            'ProfileSummary' => $details->Descriptions,
            'CVPath' => $details->File,
        );

        // Call the model function to insert data
        $this->unregister_model->insert_candidate($data);

        $dataUpdate = array(
            'Status' => '0'
        );
        $this->unregister_model->update_user($id, $dataUpdate);
    ...
    public function ajax_appliedjobs($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['alljobs'] = $this->unregister_model->listAppliedJobs($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_alljobs', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
    ...
    public function ajax_savedjobs($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['alljobs'] = $this->unregister_model->listSavedJobs($per_page_record, $page_number);
        if (isset($result['alljobs'][0]->Message))
            $result['total_records'] = 0;
        else
            $result['total_records'] = $result['alljobs'][0]->rowcount;
    ...
    public function ajax_followcompanies($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['following'] = $this->unregister_model->listFollowing($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_followcompany', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
    ...
    public function ajax_skill($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['skill'] = $this->unregister_model->listSkill($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_skill', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
            echo json_encode(array('a' => '<tr><td colspan="9" style="text-align: center;">No Records Found.</td></tr>', 'b' => ''));
    ...
    public function ajax_employment($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['employment'] = $this->unregister_model->listEmployment($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['UserID'] = $this->input->post('UserID');
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_employment', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
            echo json_encode(array('a' => '<tr><td colspan="9" style="text-align: center;">No Records Found.</td></tr>', 'b' => ''));
    ...
    public function ajax_project($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['project'] = $this->unregister_model->listProject($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['UserID'] = $this->input->post('UserID');
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_project', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
    ...
    public function ajax_certificate($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['certificate'] = $this->unregister_model->listCertificate($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['UserID'] = $this->input->post('UserID');
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_certificate', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
    ...
    public function ajax_language($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['language'] = $this->unregister_model->listLanguage($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['UserID'] = $this->input->post('UserID');
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_language', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
            echo json_encode(array('a' => '<tr><td colspan="6" style="text-align: center;">No Records Found.</td></tr>', 'b' => ''));
    ...
    public function ajax_qualification($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['qualification'] = $this->unregister_model->listQualification($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['UserID'] = $this->input->post('UserID');
        $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_education', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
            echo json_encode(array('a' => '<tr><td colspan="9" style="text-align: center;">No Records Found.</td></tr>', 'b' => ''));
    ...
    public function ajax_invited($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['data_array'] = $this->unregister_model->listInvited($per_page_record, $page_number);
        ...
        $result['UserID'] = $this->input->post('UserID');
        if ($Type == "JobPost") {
            $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_jobpostinterview', $result, TRUE);
        } else {
            $ajax_listing = $this->load->view('admin/masters/unregisteruser/ajax_listing_directinterview', $result, TRUE);
        }
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditSkill($data);
                    //print_r($res);die();
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#skill');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addskill/' . $CandidateID);
                    }
                } else {
            ...
            $this->load->view('admin/masters/unregisteruser/add_editskill', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editskill_js', $data, TRUE);
    ...
    public function editskill($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editskill/' . $CandidateID . "/" . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editskill/' . $CandidateID . "/" . $ID);
            ...
            $data['skilluser'] = $this->unregister_model->getSkillByID($ID);
            ...
            if (empty($data['skilluser'])) {
                ...
                redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/' . $CandidateID);
            }
            $data['loading_button'] = getLoadingButton();
            $data['CandidateID'] = $CandidateID;
            $this->load->view('admin/masters/unregisteruser/add_editskill', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editskill_js', NULL, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $data = $this->input->post();
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditEmployment($data);
                    //print_r($res);die();
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#employment');
                    } else {
                        $error_array = array(
                            "error_message" => $res->Message,
                            "method_name" => $res->Method,
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addemployment/' . $CandidateID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addemployment/' . $CandidateID);
            ...
            $data['designation'] = getDesignationCombobox();
            $data['CandidateID'] = $CandidateID;
            $this->load->view('admin/masters/unregisteruser/add_editemployment', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editemployment_js', $data, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function editemployment($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editemployment/' . $CandidateID . "/" . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editemployment/' . $CandidateID . "/" . $ID);
            ...
            $data['employment'] = $this->unregister_model->getEmploymentByID($ID);
            ...
            $data['CandidateID'] = $CandidateID;
            $this->load->view('admin/masters/unregisteruser/add_editemployment', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editemployment_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $data = $this->input->post();
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditProject($data);
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#project');
                    } else {
                        $error_array = array(
                            "error_message" => $res->Message,
                            "method_name" => $res->Method,
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addproject/' . $CandidateID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addproject/' . $CandidateID);
            ...
            $data['CandidateID'] = $CandidateID;
            $this->load->view('admin/masters/unregisteruser/add_editproject', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editproject_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function editproject($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editproject/' . $CandidateID . "/" . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editproject/' . $CandidateID . "/" . $ID);
            ...
            $data['project'] = $this->unregister_model->getProjectByID($ID);
            ...
            $data['CandidateID'] = $CandidateID;
            $this->load->view('admin/masters/unregisteruser/add_editproject', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editproject_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $data = $this->input->post();
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditCertificate($data);
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#certificate');
                    } else {
                        $error_array = array(
                            "error_message" => $res->Message,
                            "method_name" => $res->Method,
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addcertificate/' . $CandidateID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addcertificate/' . $CandidateID);
            ...
            $CandidateDetails = $this->unregister_model->getCandidateByID($CandidateID);
            if ($CandidateDetails->BirthYear != "") {
                $start  = $CandidateDetails->BirthYear + 10;
                $data['Year'] = GetYearList($start, 0, 'Year');
            }
            $this->load->view('admin/masters/unregisteruser/add_editcertificate', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editcertificate_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function editcertificate($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editcertificate/' . $CandidateID . "/" . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editcertificate/' . $CandidateID . "/" . $ID);
            ...
            $data['certificate'] = $this->unregister_model->getCertificateByID($ID);
            ...
            $this->load->view('admin/masters/unregisteruser/add_editcertificate', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editcertificate_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $data = $this->input->post();
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditLanguage($data);
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#language');
                    } else {
                        $error_array = array(
                            "error_message" => $res->Message,
                            "method_name" => $res->Method,
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addlanguage/' . $CandidateID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addlanguage/' . $CandidateID);
            ...
            $this->load->view('admin/masters/unregisteruser/add_editlanguage', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editlanguage_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function editlanguage($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editlanguage/' . $CandidateID . "/" . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editlanguage/' . $CandidateID . "/" . $ID);
            ...
            $data['language'] = $this->unregister_model->getLanguageByID($ID);
            ...
            $this->load->view('admin/masters/unregisteruser/add_editlanguage', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editlanguage_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $data = $this->input->post();
                    $data['ID'] = 0;
                    $res = $this->unregister_model->addeditQualification($data);
                    if (@$res->ID) {

                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/details/' . $CandidateID . '#qualification');
                    } else {
                        $error_array = array(
                            "error_message" => $res->Message,
                            "method_name" => $res->Method,
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addqualification');
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/addqualification');
            ...
            $this->load->view('admin/masters/unregisteruser/add_editeducation', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editeducation_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function editqualification($CandidateID = 0, $ID = 0)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editqualification/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/editqualification/' . $ID);
            ...
            $data['qualification'] = $this->unregister_model->getQualificationByID($ID);
            $data['designation'] = getDesignationCombobox(@$data['language']->DesignationID);
            ...
            $this->load->view('admin/masters/unregisteruser/add_editeducation', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_editeducation_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    {
        $data = $this->input->post();
        $res = $this->unregister_model->editBasicDetails($data);
    }

    public function editOtherDetails()
    {
        $data = $this->input->post();
        $res = $this->unregister_model->editOtherDetails($data);
    }
    ...
    public function add()
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $url = site_url("admin/masters/unregisteruser/add");
                    $config = array(
                        'max_width' => CANDIDATE_MAX_WIDTH,
                    ...
                    $cvurl = site_url("admin/masters/unregisteruser/add");
                    $config1 = array(
                        "max_width" => -1,
                        "max_height" => -1,
                    ...
                    $res = $this->unregister_model->insert($data);
                    if ($res == 1) {
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser');
                    } else {
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/add');
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/add');
            ...
            $this->load->view('admin/masters/unregisteruser/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_edit_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $array);
            unset($data, $array);
        } catch (Exception $e) {
            echo $e->getMessage();
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $url = site_url("admin/masters/unregisteruser/edit/" . $ID);
                    $config = array(
                        "max_width" => CANDIDATE_MAX_WIDTH,
                        "max_height" => CANDIDATE_MAX_HEIGHT,
                        'max_size' => CANDIDATE_MAX_SIZE,
                    ...
                    $cvurl = site_url("admin/masters/unregisteruser/edit/" . $ID);
                    $config1 = array(
                        "max_width" => -1,
                        "max_height" => -1,
                        'max_size' => CV_MAX_SIZE,
                    ...
                    $res = $this->unregister_model->update($data);
                    if ($res == 1) {
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser');
                    } else {
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/unregisteruser/edit/' . $ID);
            ...
            $data['candidate'] = $this->unregister_model->getCandidateByID($ID);
            ...
            $this->load->view('admin/masters/unregisteruser/add_edit', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/unregisteruser/add_edit_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $res);
            unset($data, $res);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->unregister_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function changeEmploymentStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->unregister_model->changeEmploymentStatus($this->input->post());
                if ($res) {
                    $message = ($this->input->post('status') == 1) ? label('status_active') : label('status_inactive');
                    echo json_encode(array('result' => 'success', 'message' => $message));
                } else {
    ...
    public function changepassword()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if ($password == $confirm_password) {
                $res = $this->unregister_model->changePassword();
                if ($res) {
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->unregister_model->listData(-1, 1);
        $array['result'] = array();
        if (!empty($array['data'])) {
