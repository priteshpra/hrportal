...
class Module extends Admin_Controller {
    ...
    public function index($per_page_record = 10, $page_number = 1) {
        try {
            ...
            $this->load->view('admin/masters/module/list', $res);
    ...
    public function edit($module_id = NULL) {
        try {
            ...
            $this->load->view('admin/includes/header');
            $res = array();
            $res['page'] = 'edit/' . $module_id;
            $res['module'] = $this->module_model->getModuleByID($module_id);
            if(empty($res['module'])){
            ...
            $res['parent'] = getParent($res['module']->ParentID);

            $this->load->view('admin/masters/module/add_edit', $res);
            $data = array();
    ...
    public function export_to_excel() {
        ...
        $res['module'] = $this->module_model->listmodule(-1, 1);
        $dataResult['result'] = array();
        if (!empty($res['module'])) {
            $dataResult['result'] = $res['module'];
        }
