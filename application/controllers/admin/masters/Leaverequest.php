...
class Leaverequest extends Admin_Controller {
    ...
    public function index() {
        ...
        $data['user'] = getUserCombobox(0,"Employee",label('msg_lbl_select_user'));
    ...
    public function add() {
        try {
           ...
           /*$this->form_validation->set_rules('ReportingManagerID', 'ReportingManagerID', 'trim|required');*/
            $this->form_validation->set_rules('LeaveEndDate', 'LeaveEndDate', 'trim|required');
            $this->form_validation->set_rules('LeaveDays', 'LeaveDays', 'trim|required');
			$this->form_validation->set_rules('TypeOfLeave', 'TypeOfLeave', 'trim|required');
            $this->form_validation->set_rules('LeaveHalfs', 'LeaveHalfs', 'trim|required');
            /* $this->form_validation->set_rules('LeaveDay', 'LeaveDay', 'trim|required');
            $this->form_validation->set_rules('LeaveStatus', 'LeaveStatus', 'trim|required');*/
            if ($this->input->post()) {
           ...
           /*$this->form_validation->set_rules('ReportingManagerID', 'ReportingManagerID', 'trim|required');*/
            ...
            if ($this->input->post()) {
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $data['ID'] = $ID;
                    $res = $this->leaverequest_model->update($data);
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/leaverequest');
                    } else {
                        $error_array = array(
                ...
                $res['user'] = getUserCombobox($res['leaverequest']->ReportingManagerID,"Employee",label('msg_lbl_select_user'));
