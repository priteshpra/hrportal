...
class Department extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_A_GetRoleMappingByID(" . $this->UserRoleID . ",27)");
        $tmp->next_result();
    ...
    public function index() {
		...
		$this->load->view('admin/masters/department/list', $res);
		$data['page_level_js'] = $this->load->view('admin/masters/department/list_js', NULL, TRUE);
		$data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/department/add';
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1) {
        ...
        $result['department'] = $this->department_model->listData($per_page_record, $page_number);
        if (empty($result['department']))
            $result['total_records'] = 0;
        else
            $result['total_records'] = $result['department'][0]->rowcount;

        $result['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
        if ($result['total_records'] != 0) {
            $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
            $ajax_listing = $this->load->view('admin/masters/department/ajax_listing', $result, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    $res = $this->department_model->insert($this->input->post());
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/department');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/masters/department/add');
                    }
                } else {
            ...
            $this->load->view('admin/masters/department/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/masters/department/add_edit_js', NULL, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $data['ID'] = $ID;
                    $res = $this->department_model->update($data);
                    //print_r($res);die();
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/department');
                    } else {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/department/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/department/edit/' . $ID);
            ...
            $data['loading_button'] = getLoadingButton();
            $this->load->view('admin/masters/department/add_edit', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/department/add_edit_js', NULL, TRUE);
    ...
    function changeStatus() {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->department_model->changeStatus($this->input->post());
                if($res){
    ...
    public function export_to_excel() {
        ...
        $res['department'] = $this->department_model->listData(-1, 1);

        $dataResult['result'] = array();
        if (!empty($res['department'])) {
            $dataResult['result'] = $res['department'];
        }
        $fields = array("SrNo", "Department"); 
        //Header Define
